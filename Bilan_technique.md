...
| 30/08/2026 | ⚠️ **Écart documentation/code constaté** : l'Étape 16 (§6.29, 29/08/2026) documentait la fusion des flèches ‹ › dans la rangée du titre du header Leçon (`.lesson-header-bot` supprimée). Or les captures d'écran de retour terrain de ce jour montrent l'en-tête encore sur 3 lignes : `.lesson-header-bot` était toujours présent dans `index.html`, sans règle CSS dédiée (orpheline), les commentaires de `style.css` §8 décrivant déjà le résultat visé sans que le HTML ait suivi. Cause non déterminée (session interrompue ou fichier livré non à jour) — signalé sans investigation plus poussée, hors périmètre de la présente étape. |
| 30/08/2026 | 🆕 Étape 18 : 🐞 retour terrain (captures d'écran, comparaison avec l'appli sœur espagnole) — correctif complet du header Leçon, allant au-delà du correctif visé (non appliqué) par l'Étape 16 : (1) `.lesson-header-bot` effectivement supprimée, ses flèches ‹ › rejoignent `.lesson-header-top` ; (2) bouton texte "← Modules" (`lessonBackBtn`) supprimé — doublon fonctionnel confirmé avec l'icône 📚 de la nav rapide (`navGoModules()` et `lessonGoBack()` exécutent la même action de retour) ; (3) badge cliquable "Niv. 1/2" (`lessonLevelBadge`) supprimé, même doublon ; (4) icône 🚪 Quitter déplacée dans le groupe `.back-btn-group` aux côtés de 🏠❓📚 (auparavant isolée à droite par `justify-content:space-between`) ; (5) `.lesson-nav-btn` réduit (40px→26px, bordure 2px→1.5px, police 1.35rem→1rem) pour tenir à côté du titre, gabarit calé sur l'appli sœur. Header réduit à 2 lignes. CSS mort supprimé au passage (`.lesson-level-badge` ×3 règles, `.back-btn` ×2 règles — plus aucun élément ne portait ces classes). Vérifications : `node --check app.js`, comptage accolades CSS (847→842, cohérent avec les 5 règles supprimées), comptage `<div>`/`</div>` d'`index.html` (230/229→229/228, écart préexistant inchangé, cohérent avec la suppression d'une seule paire de div). Resynchronisation complète des 3 plans internes (`app.js` : 6 497 lignes, `style.css` : 5 479 lignes, `index.html` : 1 817 lignes) — Sébastien Godet + Claude Sonnet 5 |
| 30/08/2026 | 🆕 Étape 19 : 🐞 retour terrain (2e du jour, capture d'écran) — dans l'onglet Répète, la phrase d'intro unique "🔊 Une voix te fait entendre le mot, 🎙️ un micro vérifie ce que tu dis :" du panneau Réglages audio se coupait au hasard du retour à la ligne, sans lien visuel avec les 2 badges (voix/micro) qu'elle annonce juste en dessous. Scindée en 2 lignes dédiées via une nouvelle classe `.repeat-badges-intro-line` (une par badge, chacune alignée avec le badge correspondant) au lieu d'une seule phrase continue — même logique d'harmonisation que l'Étape 18. Contenu de la traduction oromo inchangé, simple scission à la virgule d'origine (aucune retraduction, donc aucun risque d'erreur introduite sur un texte déjà validé). Vérifications : `node --check app.js`, comptage accolades CSS (842→844, cohérent avec les 2 nouvelles règles ajoutées). Resynchronisation complète des 3 plans internes (`app.js` : 6 517 lignes, `style.css` : 5 492 lignes, `index.html` : 1 817 lignes) — Sébastien Godet + Claude Sonnet 5 |
