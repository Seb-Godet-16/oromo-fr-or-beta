...
| 29/08/2026 | **Étape 6 (méthode VACHÉBO)** : audit complet des chemins de navigation qui NE passent PAS par `showScreen()` (donc pas par une transition d'écran complète) pour vérifier que la voix (TTS) et le micro (`SpeechRecognition`) sont bien coupés à chacun d'eux. Nouvelle fonction centrale `_stopAllAudioAndMic()` (combine `_stopCardAudio()` et `_stopRepeat()`, déjà existantes), appelée dans **11 points de navigation** identifiés comme des fuites totales ou partielles : retournement de carte flash (`flipCard()`), icônes de nav rapide 🏠/❓/🚪 (`goToAccueil()`, `goToGuide()`, `openQuitModal()`), bouton "Fermer l'appli" (`quitAppClose()`, ne coupait que la voix), modale Infos (`showCredits()`), icône/bouton Modules 📚 (`navGoModules()`, ne coupait que la voix), bouton retour Leçon (`lessonGoBack()`, idem), clic sur le badge "Niv. 1/2" du header Leçon, bouton "Guide" de la nav basse (`showOnboardingGuide()`, distinct de l'icône ❓ déjà couverte), et bouton "Changer de langue" de la nav basse — ce dernier appelait `showScreen()` directement en `onclick` inline dans `index.html` sans passer par aucune fonction JS : nouvelle fonction dédiée `navGoLauncher()` créée à cet effet. Le point de passage en arrière-plan (`visibilitychange`) est également renforcé : il n'appelait auparavant que `speechSynthesis.cancel()` seul, sans couper le micro ni invalider le token du drill "répéter x fois" (`_cardAudioToken`), ce qui pouvait en théorie laisser un `setTimeout` de relance se déclencher au retour au premier plan. Vérifications : `node --check` sur `app.js`, diff ligne à ligne confirmant qu'aucun fichier de données ni aucune ligne hors du périmètre de l'étape n'a été touché, comptage programmatique des 11 nouveaux points d'appel (aucun oubli ni doublon par rapport à la liste auditée), smoke-test isolé (harnais Node `vm` avec DOM minimal simulé) exécutant chacune des fonctions modifiées pour détecter toute exception avant le test manuel en conditions réelles (`app.js` : 6 414 lignes, `index.html` : 1 819 lignes — 1 seule ligne modifiée) (Sébastien + Claude Sonnet 5) |
| 29/08/2026 | **Étape 7 (méthode VACHÉBO)** : 🐞 retour terrain (capture d'écran) — le header de l'écran Leçon prenait 3 lignes (rangée d'icônes rapides 🏠/❓/📚, puis titre, puis une ligne à part rien que pour les flèches ‹ › de nav entre modules), contre 2 sur l'appli sœur espagnole. Fusion des flèches dans la même rangée que le titre (`.lesson-header-bot` supprimée, son contenu déplacé dans `.lesson-header-top`, juste après `#lessonTitle`) — le titre continue de passer seul à la ligne s'il est long, mais les flèches ne forment plus une rangée à part, ce qui harmonise l'affichage avec l'appli sœur et regagne la hauteur d'une ligne complète d'en-tête. `#lessonTitle` reçoit `min-width:0` (nécessaire pour qu'un `flex:1` se comprime correctement une fois les flèches dans la même rangée). Vérifications : `node --check app.js`, comptage accolades CSS (844→845, cohérent avec l'ajout net d'une règle CSS), comptage de balises `<div>`/`</div>` avant/après dans `index.html` (`230`/`229` → `229`/`228`, écart préexistant inchangé — un `<div>` et sa fermeture ont bien été retirés ensemble) (`index.html` : 1 821 lignes, `style.css` : 5 491 lignes) (Sébastien + Claude Sonnet 5) |
| 29/08/2026 | **Étape 8 (méthode VACHÉBO)** : 🐞 retour terrain (captures d'écran) — deux correctifs distincts sur l'affichage des cartes de vocabulaire. **1)** Taille de la flashcard (onglet Cartes) et de la carte du mot (onglet Répète) désormais adaptée au contenu réel plutôt qu'une hauteur fixe unique pour tous les mots : nouvelle fonction `_adjustFlashcardHeight()`, appelée après chaque rendu de carte (et au redimensionnement du viewport), qui mesure la hauteur naturelle du recto ET du verso (`.scrollHeight`, fiable même superposées en `position:absolute`) et redimensionne `.fc-wrap` en conséquence — carte compacte pour un mot simple, plus grande pour un verbe à conjugaisons ; si le contenu dépasse même le plafond dérivé du viewport (verbe à conjugaison très longue sur petit écran), `.lesson-body` redevient scrollable le temps de cette carte (classe `.fc-tall`) au lieu d'un scroll interne à la carte — même technique déjà en place pour le panneau Réglages audio ouvert (`.audio-settings-open`, voir Bilan technique §6.20/§6.28). Carte de l'onglet Répète (`.repeat-card`) : plancher de hauteur et padding réduits (130px/24px → 96px/18px) pour ne plus laisser un vide disproportionné sur une entrée sans indice long. **2)** La phrase d'intro "🔊 Une voix te fait entendre le mot, 🎙️ un micro vérifie ce que tu dis" (onglet Répète) vit désormais **dans** le panneau Réglages audio, juste au-dessus des 2 badges qu'elle explique (voix TTS + reconnaissance micro), au lieu de rester seule au-dessus de la carte — elle en est le titre depuis que ces 2 badges ont rejoint ce panneau (étape précédente du 29/08), et se retrouvait orpheline, parfois coupée par le header sticky au scroll. Vérifications : `node --check app.js`, comptage accolades CSS (845/845). Point signalé pour recettage terrain : le cas `.fc-tall` (carte à conjugaison dépassant même le plafond viewport) n'a pu être raisonné que sur le code, faute d'accès à un navigateur réel dans cet environnement — à vérifier par Sébastien sur un verbe à conjugaison longue en petit écran (`app.js` : 6 505 lignes, `style.css` : 5 513 lignes) (Sébastien + Claude Sonnet 5) |
| 30/08/2026 | 🐞 Correctif retour terrain (captures d'écran, comparaison directe avec l'appli sœur espagnole) — l'en-tête de l'écran Leçon tenait encore sur 3 lignes malgré l'Étape 7 du 29/08 censée l'avoir réduit à 2 : `.lesson-header-bot` était en réalité toujours présent dans `index.html` (orphelin, sans règle CSS dédiée), écart entre la doc et le code non expliqué. Correctif complet cette fois, avec 3 suppressions supplémentaires demandées en retour terrain : bouton texte "← Modules" (`lessonBackBtn`) retiré — doublon confirmé avec l'icône 📚 nav rapide, `navGoModules()` et `lessonGoBack()` effectuant la même action ; badge cliquable "Niv. 1/2" (`lessonLevelBadge`) retiré, même doublon ; icône 🚪 Quitter déplacée dans le groupe `.back-btn-group` aux côtés de 🏠❓📚 (n'est plus isolée à droite) ; flèches ‹ › de nav entre modules déplacées à droite du titre, sur sa ligne, et réduites (40px→26px) pour s'y intégrer sans écraser le titre, gabarit calé sur l'appli sœur espagnole. CSS mort supprimé au passage (`.lesson-level-badge` et `.back-btn`, 5 règles au total, plus aucun élément ne les utilisant). Vérifications : `node --check app.js`, comptage accolades CSS (847→842, cohérent), comptage `<div>`/`</div>` d'`index.html` (230/229→229/228, écart préexistant inchangé, cohérent avec le retrait d'une seule paire de div). Resynchronisation complète des 3 plans internes — voir Bilan technique, entrée du 30/08/2026 (`app.js` : 6 497 lignes, `style.css` : 5 479 lignes, `index.html` : 1 817 lignes) (Sébastien + Claude Sonnet 5) |
| 30/08/2026 | 🐞 Correctif retour terrain (2e du jour, capture d'écran) — dans l'onglet Répète, la phrase d'intro "🔊 Une voix te fait entendre le mot, 🎙️ un micro vérifie ce que tu dis :" (au-dessus des 2 badges du panneau Réglages audio) était une seule phrase continue, qui se coupait au hasard du retour à la ligne (ex. "...ce que tu / dis :") sans lien visuel avec les 2 badges qu'elle annonce juste en dessous. Scindée en 2 lignes dédiées (`.repeat-badges-intro-line`) : une pour la voix 🔊, une pour le micro 🎙️, chacune alignée avec son badge correspondant — même principe d'harmonisation que le header Leçon du correctif précédent. Traduction oromo non modifiée dans son contenu, seulement scindée à la virgule d'origine (aucun risque d'erreur de traduction introduite). Vérifications : `node --check app.js`, comptage accolades CSS (842→844, cohérent avec les 2 nouvelles règles `.repeat-badges-intro-line`). Resynchronisation complète des 3 plans internes (`app.js` : 6 517 lignes, `style.css` : 5 492 lignes, `index.html` : 1 817 lignes) (Sébastien + Claude Sonnet 5) |
| 30/08/2026 | 📝 Remerciements enrichis (3e du jour) — l'appli sœur espagnole VACHÉBO crédite Mussa Sembro pour un retour UX direct sur Taphad'Meuh (interface trop chargée pour de grands débutants). Confirmé par le porteur du projet : Mussa est à l'origine du **constat**, pas de la **solution** (regroupement en catégories, §6.27, décision du porteur du projet). Ajout d'un retour terrain sobre — "traduction, conseils linguistiques et retour terrain (interface trop chargée pour de grands débutants)" — dans les **4 emplacements "remerciements"** de l'app (hors "Qui suis-je ?"/"Eenyu ani?", hors périmètre) : bullets FR + brouillon Oromo de la modale crédits (`app.js` §18/`showCredits()`), paragraphes FR "Remerciements" + brouillon Oromo "Galateeffannaa" du Guide (`index.html`, écran `#home` §8). Pour `index.html` FR, la phrase existante et floue ("conseils sur l'ergonomie", attribuée conjointement à Mussa et aux parents) est conservée intacte — portée plus large, sujet distinct — et le nouveau retour terrain est ajouté en phrase séparée, spécifique à Mussa. Traductions oromo provisoires (IA), en attente de relecture par Mussa Sembro comme le reste du lot en cours. Resynchronisation des plans internes `app.js` (§18 à §21c) et `index.html` (Écran 2a à Scripts) par recherche directe — a révélé une dérive préexistante d'environ 320-330 lignes sur les ancres `app.js` §18+, non liée à ce correctif, non corrigée au-delà de §18-§21c (voir Bilan technique §6.31 et point de vigilance §7). Vérifications : `node --check app.js`, comptage `<div>`/`</div>` d'`index.html` (229/228 → 229/228, inchangé), comptage des mentions "Mussa" (`app.js` 10→11, `index.html` 5→8), 4 occurrences exactes de la nouvelle mention sans doublon, miroir FR/OR confirmé sur le sens (constat crédité, solution non attribuée) (`app.js` : 6 548 lignes, `index.html` : 1 851 lignes) (Sébastien + Claude Sonnet 5) |
| 31/08/2026 | 🐞📝 4 correctifs retour terrain, transmis via document Word annoté de 5 captures d'écran ("Faire_les_modifications_suivantes") — voir Bilan technique §6.32 pour le détail complet. **(1)** Auto-dépli du démarrage : `_computeAutoOpenCategoryIds()` (`app.js` §7) ouvrait la 1ʳᵉ catégorie non terminée **et** la suivante — n'ouvre plus que la 1ʳᵉ, pour le Niveau 1 comme le Niveau 2. **(2)** Icône 👆 + phrase "Cliquez pour voir la traduction en français" retirées du recto des flashcards (doublon avec le libellé Recto/Verso déjà affiché au-dessus) ; classe CSS `.fc-front-hint` orpheline supprimée de `style.css` (base + mode sombre). **(3)** Bullet Mussa Sembro (modale crédits) reformulé : "vérification et correction de la traduction oromo & retour terrain" (remplace le texte de §6.31), FR + brouillon Oromo. **(4)** Paragraphe Remerciements du Guide (introduit en §6.31) scindé en 2 paragraphes distincts : un pour Mussa Sembro (texte aligné sur le point 3, avec le détail "interface un peu trop chargée pour de grands débutants" conservé ici), un pour **mes parents** (gras, relecture + ergonomie) — motif du porteur du projet : "mes parents c'était bien plus global". FR + brouillon Oromo. Vérifications : `node --check app.js`, comptage `<div>`/`</div>` d'`index.html` (229/228, inchangé), comptage accolades CSS (842/842, inchangé après retrait de 2 règles `.fc-front-hint`). Resynchronisation complète des plans internes `app.js` (§7 à §21c — la dérive préexistante signalée en §6.31 est résorbée sur ce périmètre) et `index.html` (Écran 2a à Scripts), par recherche directe, à convergence (`app.js` : 6 549 lignes, `style.css` : 5 490 lignes, `index.html` : 1 864 lignes) (Sébastien + Claude Sonnet 5) |
